...
bool Bureaucrat::signForm(Form &form) const
{
    ...
    {
        std::cout << RED << "Error: " << this->getName() << "(Bureaucrat) cannot sign " << form.getName() << "(Form) because " << e.what() << RESET << std::endl;
        return false;
    }
    return true;
}
...
bool Bureaucrat::executeForm(Form const &form) const
{
    ...
    {
        form.execute(*this);
        std::cout << GREEN << this->getName() << "(Bureaucrat) executes " << form.getName() << "(Form)" << RESET << std::endl;
    } 
    catch (std::exception const &e)
    {
        std::cout << RED << "Error: " << this->getName() << "(Bureaucrat) cannot execute " << form.getName() << "(Form) because " << e.what() << std::endl;
        return false;
    }
    return true;
}
